...
- `references/subagent-design.md` — for long-running or context-heavy automation
- `references/failure-modes.md` — hook failure modes (`block: false` is a no-op; `ownsCompaction: true` + no-op compact disables overflow recovery)
- `references/security-audit.md` — review hook safety before install from ClawHub
- `assets/worksheets/heartbeat-cost-calculator.md` — pre-budgeted cadence table before enabling heartbeat
- `assets/templates/plugin-skeleton-hook-only/` — starter for an install-gated hook plugin
