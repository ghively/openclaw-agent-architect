...
- `references/multi-agent-routing.md` — separate agents vs. subagents; when to pick which
- `references/automation-and-hooks.md` — cron-driven work as an alternative to subagents
- `references/tool-policy-and-security.md` — scoped tool profiles and sandboxing for child contexts
- `references/failure-modes.md` — verbatim pass-through and orphaned-session symptoms
- `SKILL.md` — the Execute-Verify-Report pattern the parent relies on when spawning
