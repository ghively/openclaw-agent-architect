...
Elicitation is a high-uncertainty conversation — users drop channel names, MCP servers, APIs, and model IDs you may not have fresh memory of. Whenever a user mentions a specific capability, feature, or service whose current behavior will drive a design decision, **pause and research before continuing**.

The discipline, scripts, primary sources, and failure modes are in **`references/research-discipline.md`** — consult it for the full treatment. Summary for elicitation:

- If a user-mentioned specific matters to the design, fetch `docs.openclaw.ai` (or the service's docs) rather than guessing.
- Batch uncertain items in a single research pass when possible — one pause, not three.
- Tell the user what you checked: "Just pulled the Telegram docs and confirmed voice-in is supported; that changes dimension 3's answer."
...
- `references/plan-mode.md` — elicitation sits inside the plan-mode contract
- `references/research-discipline.md` — canonical home for the research-pause rule
- `references/system-architecture.md` — the next step after elicitation
- `references/build-tracks.md` — pre-flight (dims 1–2) drives track selection
- `assets/worksheets/tool-posture-wizard.md` — fast track picker once dims 1–2 are locked
